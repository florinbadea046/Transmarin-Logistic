...
const handleSave = () => {
  ...
  toast.success(`${newInvoices.length} factur${newInvoices.length === 1 ? "ă" : "i"} importate cu succes`);
};
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <Select value={tipFilter} onValueChange={setTipFilter}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="venit">
                      {t("invoices.typeLabels.income")}
                    </SelectItem>
                    <SelectItem value="cheltuială">
                ...
                <Select value={statusFilter} onValueChange={setStatusFilter}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="plătită">
                      {t("invoices.statusLabels.plătită")}
                    </SelectItem>
                    <SelectItem value="neplatită">
                      {t("invoices.statusLabels.neplatită")}
                    </SelectItem>
                    <SelectItem value="parțial">
                      {t("invoices.statusLabels.parțial")}
                    </SelectItem>
                    <SelectItem value="anulată">
                      {t("invoices.statusLabels.anulată")}
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>
                      {t("invoices.columns.clientSupplier")}
                    </TableHead>
                    <TableHead className="text-right">
                      {t("invoices.columns.subtotal")}
                    </TableHead>
                    <TableHead className="text-right">
                      {t("invoices.columns.vat")}
                    </TableHead>
                    <TableHead className="text-right">
                    ...
                    filtered.map((inv) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell>
                            <Badge
                              className={`border ${statusColor[inv.status]}`}
                            >
                              {t(`invoices.statusLabels.${inv.status}`)}
                            </Badge>
                          </TableCell>
        ...
        <DialogContent className="w-full max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-2">
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                value={form.scadenta}
                onChange={(e) =>
                  setForm((prev) => ({ ...prev, scadenta: e.target.value }))
                }
              />
            </div>
            <div className="space-y-1 col-span-1 sm:col-span-2">
              ...
              <Select
                value={form.clientFurnizor}
                onValueChange={(v) =>
                  setForm((prev) => ({ ...prev, clientFurnizor: v }))
                }
              >
                <SelectTrigger>
                  <SelectValue />
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" variant="outline" onClick={addLine}>
                <Plus className="w-3 h-3 mr-1" /> {t("invoices.form.addLine")}
              </Button>
            </div>
                ...
                <div key={linie.id} className="rounded-md border p-3 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-muted-foreground">
                      {t("invoices.form.itemNumber", { number: idx + 1 })}
                    </span>
                    <Button
                      size="icon"
                      variant="ghost"
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateLine(idx, "descriere", e.target.value)
                      }
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          updateLine(idx, "cantitate", Number(e.target.value))
                        }
                      />
                    </div>
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          updateLine(idx, "pretUnitar", Number(e.target.value))
                        }
                      />
                    </div>
                  </div>
            ...
            <div className="flex flex-col items-end gap-1 text-sm pr-2">
              <div className="flex gap-4 sm:gap-8">
                ...
                <span className="font-medium w-28 sm:w-32 text-right">
                  {formatCurrency(totals.totalFaraTVA)}
                </span>
              </div>
              <div className="flex gap-4 sm:gap-8">
                <span className="text-muted-foreground">
              ...
              <div className="flex gap-4 sm:gap-8 text-base">
                <span className="font-semibold">
                  {t("invoices.totals.total")}:
                </span>
                <span className="font-bold w-28 sm:w-32 text-right">
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            ...
            <Button className="w-full sm:w-auto" onClick={handleSave}>
              {editId
                ? t("invoices.actions.saveChanges")
                : t("invoices.actions.addInvoice")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
