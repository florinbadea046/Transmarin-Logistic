...
export function InvoiceFormDialog({ dialogOpen, setDialogOpen, editId, form, setForm, handleSave, updateLine, addLine, removeLine, totals }: { dialogOpen: boolean; setDialogOpen: (open: boolean) => void; editId: string | null; form: FormState; setForm: Dispatch<SetStateAction<FormState>>; handleSave: () => void; updateLine: (idx: number, field: keyof InvoiceLine, value: string | number) => void; addLine: () => void; removeLine: (idx: number) => void; totals: { totalFaraTVA: number; tva: number; total: number } }) {
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
      <DialogContent className="w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            {form.linii.map((linie, idx) => (
              <div key={linie.id} className="rounded-md border p-3 space-y-2">
                ...
                <div className="flex justify-end text-sm font-semibold">
                  {t("invoices.columns.total")}: {formatCurrency(linie.cantitate * linie.pretUnitar)}
                </div>
              </div>
            ))}
          </div>
        ...
        <DialogFooter className="flex-col sm:flex-row gap-2">
          ...
          <Button className="w-full sm:w-auto" onClick={handleSave} disabled={form.linii.length === 0 || form.linii.every((l) => !l.descriere.trim())}>
            {editId ? t("invoices.actions.saveChanges") : t("invoices.actions.addInvoice")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
