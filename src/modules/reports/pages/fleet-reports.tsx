...
// ── Types ─────────────────────────────────────────────────────────────────────
interface ServiceRecord { truckId: string; cost: number; date: string; }
interface FuelRecord { truckId: string; liters: number; cost: number; date: string; }
interface TruckReport {
  id: string; plateNumber: string; brand: string; model: string; mileage: number;
  serviceCount: number; serviceCost: number; fuelLiters: number; fuelCost: number;
  totalCost: number; avgFuelConsumption: number;
}
...
export default function FleetReportsPage() {
  ...
  const reports: TruckReport[] = useMemo(() => {
    return TRUCKS.map((truck) => {
      ...
      return {
        id: truck.id, plateNumber: truck.plateNumber, brand: truck.brand,
        model: truck.model, mileage: truck.mileage, serviceCount: svc.length,
        serviceCost, fuelLiters, fuelCost, totalCost: serviceCost + fuelCost,
        avgFuelConsumption: fuelLiters > 0
          ? Math.round((fuelLiters / (truck.mileage / 100)) * 10) / 10 : 0,
      };
    });
  }, [monthFilter]);
  ...
  const handleTruckFilter = (v: string) => { setTruckFilter(v); setPage(1); };
  const handleMonthFilter = (v: string) => { setMonthFilter(v); setPage(1); };
  const handleSearch = (v: string) => { setSearch(v); setPage(1); };
  const handleSort = (key: SortKey) => { setSortBy(key); setPage(1); };
    ...
    <>
      ...
      <Main>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Card>
              <CardContent className="flex items-center gap-4 pt-6">
                <div className="p-2 rounded-lg bg-blue-500/10"><Wrench className="w-5 h-5 text-blue-400" /></div>
                <div>
            ...
            <Card>
              <CardContent className="flex items-center gap-4 pt-6">
                <div className="p-2 rounded-lg bg-orange-500/10"><Fuel className="w-5 h-5 text-orange-400" /></div>
                <div>
          ...
          <Card>
            <CardHeader><CardTitle>Cost per Vehicul</CardTitle></CardHeader>
            <CardContent>
              {/* Filters */}
              <div className="flex flex-col sm:flex-row gap-3 mb-6">
                <div className="relative flex-1">
                  <Truck className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                  <Input className="pl-9" placeholder="Caută camion..." value={search}
                    onChange={(e) => handleSearch(e.target.value)} />
                </div>
                <Select value={truckFilter} onValueChange={handleTruckFilter}>
                  <SelectTrigger className="w-full sm:w-52"><SelectValue placeholder="Toate camioanele" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="toate">Toate camioanele</SelectItem>
                    {TRUCKS.map((t) => (
                      <SelectItem key={t.id} value={t.id}>{t.plateNumber} — {t.brand} {t.model}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                <Select value={monthFilter} onValueChange={handleMonthFilter}>
                  <SelectTrigger className="w-full sm:w-44"><SelectValue placeholder="Toate lunile" /></SelectTrigger>
                  <SelectContent>
                  ...
                  return (
                    <div key={r.id} className="rounded-lg border p-4 space-y-3">
                      <div className="flex items-start justify-between">
                        ...
                        {globalIdx === 0 && (
                          <Badge className="bg-red-500/20 text-red-400 border-red-500/30 border text-xs">Top cost</Badge>
                        )}
                      </div>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between mt-4">
                  <p className="text-sm text-muted-foreground">
                    {`${(page - 1) * PAGE_SIZE + 1}–${Math.min(page * PAGE_SIZE, filtered.length)} din ${filtered.length} vehicule`}
                  </p>
