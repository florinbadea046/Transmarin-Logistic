...
const TRUCK_COL_MAP: Record<string, string> = {
  "numar inmatriculare": "plateNumber", "platenumber": "plateNumber", "plate": "plateNumber", "numar": "plateNumber",
  "marca": "brand", "brand": "brand",
  "model": "model",
  "an": "year", "an fabricatie": "year", "year": "year",
  "kilometraj": "mileage", "mileage": "mileage", "km": "mileage",
  "itp": "itpExpiry", "expirare itp": "itpExpiry", "itpexpiry": "itpExpiry",
  "rca": "rcaExpiry", "expirare rca": "rcaExpiry", "rcaexpiry": "rcaExpiry",
  "vigneta": "vignetteExpiry", "expirare vigneta": "vignetteExpiry", "vignetteexpiry": "vignetteExpiry",
};
...
function TruckImportDialog({
  ...
  const existing = React.useMemo(() => getCollection<Truck>(STORAGE_KEYS.trucks), [open]);
      ...
      <DialogContent className={cn("flex flex-col gap-4", isMobile ? "max-w-[calc(100vw-2rem)] p-4" : "max-w-2xl")}>
        ...
        {rows.length === 0 ? (
          ...
          >
            ...
            <div>
              <p className="text-sm font-medium text-foreground">{t("trucks.import.dropzone")}</p>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-2 rounded-lg border bg-muted/30 px-3 py-2">
              ...
              <Button variant="ghost" size="icon" className="h-7 w-7 shrink-0" onClick={reset}><X className="h-3.5 w-3.5" /></Button>
            </div>
            ...
            <div className={cn("grid gap-2", isMobile ? "grid-cols-1" : "grid-cols-3")}>
              <div className="flex items-center gap-2 rounded-lg border border-green-200 bg-green-50 dark:bg-green-950 px-3 py-2">
                ...
                <span className="text-sm">{t("trucks.import.validCount", { count: validRows.length })}</span>
              </div>
              <div className={cn("flex items-center gap-2 rounded-lg border px-3 py-2",
                ...
                <span className="text-sm">{t("trucks.import.invalidCount", { count: invalidRows.length })}</span>
              </div>
              <div className={cn("flex items-center gap-2 rounded-lg border px-3 py-2",
                ...
                <span className="text-sm">{t("trucks.import.duplicateCount", { count: duplicateRows.length })}</span>
              </div>
            </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="min-w-[120px]">{t("trucks.columns.plateNumber")}</TableHead>
                      <TableHead className="min-w-[100px] hidden sm:table-cell">{t("trucks.fields.brand")}</TableHead>
                      <TableHead className="min-w-[200px]">{t("trucks.import.errorsCol")}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {rows.map((row) => (
                      <TableRow key={row.rowIndex} className={cn(
                        ...
                        <TableCell className="text-sm font-medium whitespace-nowrap">{row.mapped.plateNumber ?? "—"}</TableCell>
                        <TableCell className="hidden sm:table-cell text-sm whitespace-nowrap">{String(row.mapped.brand ?? "—")}</TableCell>
                        <TableCell className="text-xs whitespace-nowrap">
                          {row.errors.length > 0
                            ? <span className="text-red-600">{row.errors.join(", ")}</span>
                            : row.isDuplicate
                            ? <span className="text-yellow-600">{t("trucks.import.duplicate")}</span>
        ...
        <DialogFooter className={cn(isMobile && "flex-col gap-2")}>
          <Button variant="outline" onClick={() => onOpenChange(false)} className={cn(isMobile && "w-full")}>{t("trucks.cancel")}</Button>
