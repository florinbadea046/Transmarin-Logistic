...
import { getAutoAssignSuggestions } from "@/utils/auto-assign";
...
function SuggestButton({
  ...
  drivers: import("@/modules/transport/types").Driver[];
  trucks: import("@/modules/transport/types").Truck[];
  allTrips: import("@/modules/transport/types").Trip[];
  onSelect: (driverId: string, truckId: string) => void;
  t: ReturnType<typeof useTranslation>["t"];
}) {
  const [open, setOpen] = React.useState(false);
  const suggestions = React.useMemo(
    () => getAutoAssignSuggestions(drivers, trucks, allTrips, 3),
    [drivers, trucks, allTrips],
  );
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          {t("trips.suggest.button")}
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-80 p-3" align="start">
        <p className="text-sm font-medium mb-2">
          {t("trips.suggest.title")}
        </p>
        {suggestions.length === 0 ? (
          <p className="text-xs text-muted-foreground">
            {t("trips.suggest.noSuggestions")}
          </p>
        ) : (
          <div className="flex flex-col gap-2">
            {suggestions.map((s, i) => (
              <button
                key={`${s.driver.id}-${s.truck.id}`}
                type="button"
                onClick={() => {
                  onSelect(s.driver.id, s.truck.id);
                  setOpen(false);
                }}
                className="w-full text-left rounded-lg border p-2.5 hover:bg-muted transition-colors text-sm"
              >
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {s.score} km {t("trips.suggest.thisMonth")}
                  </span>
                </div>
                <div className="text-xs text-muted-foreground space-y-0.5">
                  <div>
                    🚛 {s.truck.plateNumber} · {s.truck.brand} {s.truck.model}
                  </div>
                  <div>
                    👤 {t("trips.suggest.driverKm")}: {s.driverKmThisMonth} km
                    &nbsp;·&nbsp;
                    🚚 {t("trips.suggest.truckKm")}: {s.truckKmThisMonth} km
                  </div>
                </div>
              </button>
            ))}
