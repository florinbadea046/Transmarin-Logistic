...
function OrgNode({ emp, all, onClick, expanded, toggle, visited = new Set() }: {
  emp: Employee;
  all: Employee[];
  ...
  const children = all.filter((e) => e.managerId === emp.id && !visited.has(e.id));
    ...
    <div className="flex flex-col items-center min-w-[140px]">
      ...
      {children.length > 0 && isExpanded && (
        <div className="flex flex-row gap-4 mt-3">
          {children.map((child) => (
            <OrgNode key={child.id} emp={child} all={all} onClick={onClick} expanded={expanded} toggle={toggle} visited={new Set(visited).add(emp.id)} />
          ))}
        </div>
      )}
    </div>
...
const OrgChart: React.FC = () => {
  ...
  if (roots.length === 0) return <div className="text-muted-foreground">Nu există angajați definiți.</div>;
  ...
  return (
    <div>
      ...
      <div ref={scrollRef} className="overflow-x-auto py-4">
        <div
        ...
        >
          <div className="flex flex-row gap-6 justify-center">
            {roots.map((root) => (
              <OrgNode key={root.id} emp={root} all={employees} onClick={setSelected} expanded={expanded} toggle={handleToggle} />
            ))}
          </div>
        </div>
      </div>
