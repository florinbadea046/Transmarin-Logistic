import { Header } from "@/components/layout/header";
import { Main } from "@/components/layout/main";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";

export default function LeavesPage() {
  return (
    <>
      <Header>
        <h1 className="text-lg font-semibold">Concedii</h1>
      </Header>
      <Main>
        <Card>
          <CardHeader>
            <CardTitle>Gestiune Concedii</CardTitle>
          </CardHeader>
          <CardContent className="flex h-96 items-center justify-center text-muted-foreground">
            <div className="text-center space-y-2">
              <p className="text-lg font-medium">TODO: Implementați aici</p>
              <ul className="text-sm text-left list-disc pl-4 space-y-1">
                <li>Lista cereri de concediu cu status</li>
                <li>Formular cerere concediu nouă</li>
                <li>Aprobare / respingere cereri</li>
                <li>Calendar cu concediile echipei</li>
                <li>Calcul zile rămase per angajat</li>
              </ul>
            </div>
          </CardContent>
        </Card>
      </Main>
    </>
  );
}
