...
function HrAuditEntryCard({ entry, isMobile }: { entry: HrAuditEntry; isMobile: boolean }) {
  ...
  const dotColor =
    ...
    entry.action === "approve" ? "border-emerald-400 text-emerald-600" :
    entry.action === "reject" ? "border-orange-400 text-orange-600" :
...
export default function ActivityLogHRPage() {
  ...
  const [filterEntity, setFilterEntity] = React.useState<HrAuditEntity | "all">("all");
  const [filterAction, setFilterAction] = React.useState<HrAuditAction | "all">("all");
  ...
  const filtered = React.useMemo(() => {
    return entries.filter((e) => {
      ...
      if (filterEmployee !== "all") {
        const empName = employees.find((emp) => emp.id === filterEmployee)?.name;
        if (!empName) return false;
        if (e.entityId !== filterEmployee && !e.entityLabel.includes(empName)) return false;
      }
      return true;
    });
  ...
  const hasFilters = filterEntity !== "all" || filterAction !== "all" || filterEmployee !== "all";
    ...
    <>
      ...
      <Main>
        ...
        <div className={cn(
          ...
          <Select value={filterAction} onValueChange={(v) => setFilterAction(v as HrAuditAction | "all")}>
        ...
        {paginated.length === 0 ? (
          ...
          <div className="min-w-0">
            {paginated.map((entry) => (
              <HrAuditEntryCard key={entry.id} entry={entry} isMobile={isMobile} />
            ))}
          </div>
        )}
